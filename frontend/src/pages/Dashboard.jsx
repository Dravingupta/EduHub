...
const Dashboard = () => {
    ...
    return (
        <div className="flex flex-col md:flex-row min-h-screen bg-[#0E0E0E] text-[#F5F5F5]">
            {/* Sidebar / Top Nav */}
            <aside className="w-full md:w-[250px] border-b md:border-b-0 md:border-r border-[#262626] p-4 md:p-6 flex flex-col md:min-h-screen md:sticky md:top-0 bg-[#0E0E0E] z-10 shrink-0 animate-fade-in">
            ...
            {/* Main Content Area */}
            <main className="flex-1 flex flex-col p-4 md:p-8 w-full max-w-full overflow-hidden animate-slide-up" style={{ animationDelay: '0.1s', animationFillMode: 'both' }}>
                {/* Top bar */}
                <header className="w-full flex justify-between items-center border-b border-[#262626] pb-4 md:pb-6 mb-6">
                    ...
                    <button
                        onClick={logout}
                        className="px-4 py-2 bg-[#161616] text-[#F5F5F5] border border-[#262626] rounded hover:bg-[#262626] transition-colors shrink-0 text-sm cursor-pointer hover:scale-105"
                    >
                        Logout
                    </button>
                </header>
